...
Before replacing an active image, ArrTags must retain plugin-owned source-artwork
provenance sufficient to avoid repeatedly overlaying its own output and to
restore the original source when appropriate. Restoration must not overwrite a
new manual image that is no longer identified as ArrTags-owned.
...
- ArrTags must track source identity, generated-artwork identity, configuration,
  renderer, and metadata fingerprints.
