...
(`/api/v3/movie` or `/api/v3/series`) fails the whole reconciliation window:
because one population serves every work item in the window, the reader returns
the bounded failure and caches nothing, and each work item that waited on the
per-connection gate re-checks the still-empty cache and re-attempts the
population, so the failure blast radius is wider than the pre-11.2 per-item
library read. The per-item bounded last-known-good metadata state is what retains
