...
current inventory. The ArrTags-side invalidation sources are later work (task
11.3), so the complete trigger set is not yet wired and the TTL remains the only
...
bounds. The observation set's `ObservedAt` is the population time, and the cache
