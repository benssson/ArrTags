...
(ADR-018 clauses 3 and 7). The provider-client integration and the ArrTags-side
invalidation sources are later work, so the boundary is defined and bounded here
while the provider reads are unchanged.
...
fallback (ADR-018 clause 3). No provider `ETag` or revision token is assumed
(ADR-018 clause 7).
