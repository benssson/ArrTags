...
inventory TTL is the total bounded lifetime of one observation set (fresh for
the first half and explicit bounded last-known-good for the remaining half), so
a provider failure keeps the bounded last-known-good inventory without extending
the window; an observation set that exceeds the configured record or byte bound
is rejected and the caller keeps the direct provider read unchanged. No provider
`ETag`, `If-None-Match`, revision token, or `history/since` watermark is assumed
(ADR-018 clauses 3 and 7).
...
existing direct read unchanged, and a transient provider failure keeps the
bounded last-known-good observation set until the TTL. On the cached population
...
per-item file reads. The direct-read fallback (over-bound sets and failed bulk
reads) remains and still performs the per-record file read unchanged. The
...
the remaining half, and expired and unusable as current at or after the TTL, so
a provider failure keeps the bounded last-known-good inventory without extending
the window. The per-connection record and byte bounds reject an over-bound
