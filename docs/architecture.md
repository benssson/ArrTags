...
identity-neutral. The value filter is applied in the documented ADR-017 order
...
renderer configuration fingerprint and the render fingerprint; the V1 default is
identity-neutral, so an unchanged configuration keeps the V1 identity. The
scaled 24-pixel inset and all ADR-009 safe-area, text-limit, contrast, opacity,
and determinism guarantees are unchanged, and a size or position that cannot fit
...
`RendererConfiguration.Validate`. Renderer validation uses
