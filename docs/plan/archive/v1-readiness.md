...
provider DTOs or turns missing values into claims. (ADR-019 supersedes the fixed
bottom-left rail, top-right status pill, and code-owned size: v1.1 makes the
global rail anchor and preset size configurable while keeping the reference
geometry, safe area, text limits, contrast, opacity, and determinism guarantees.)
...
| 6 | Define catalogue caching, provider inventory, provider-version support, and cross-provider normalization. | Resolved for v1.1 | Resolved by ADR-013 (provider-version support and optional-field policy) and ADR-018 (provider inventory cache). Task 11.1 defines the canonical, secret-free cache model and its validated bounds (`docs/data-model.md` section 6); task 11.2 populates and consumes it at the provider-client boundary so one library read serves a reconciliation window and the bulk file-selection endpoints replace per-item reads; task 11.3 wires the ArrTags-side invalidation across the documented trigger set; task 11.4 reconciles the named documents with the shipped cache behavior, records limitation F1 as resolved, and adds the Goal C integration coverage for the one-read-per-window, invalidation-source, bounds, and last-known-good facets. Cross-provider normalization remains a post-V1 consideration. |
...
- Define the Sonarr catalogue cache, provider inventory strategy, initial
  ...
  `ProviderIncompatible`. The provider catalogue/inventory cache is implemented
  in v1.1 (ADR-018, Phase 11 tasks 11.1-11.4) with ArrTags-side invalidation and
  the recorded F1 resolution, and the cross-provider normalization strategy
  remains a post-V1 consideration.
...
reviews; consolidated in `docs/limitations.md`).** The v1.1-promoted items below
are annotated as closed; the remaining non-promoted items stay open and are not
presented as solved:
...
- **Promoted to v1.1 (ADR-018) and complete (Phase 11):** provider
  catalogue/inventory cache. The bounded, secret-free, short-TTL inventory cache
  with ArrTags-side invalidation is implemented (tasks 11.1-11.3), and the Goal C
  documentation reconciliation and integration verification are recorded (task
  11.4), so the "avoid unnecessary API requests" goal is met for the provider
  library read within a window; limitation F1 is resolved.
