...
- **Promoted to v1.1 (ADR-016):** runtime configuration replacement wiring. `ConfigurationSnapshotService.TryReplace`
  ...
  resolve their values from the current snapshot per operation. The bounded
  post-save re-render trigger (v1.1 task 9.4) and the final Goal A verification
  (task 9.5) remain.
