...
task 14.3. Phase 12 (Badge value allowlist and badge size/position) is complete:
...
pinned-host confirmation is owned by task 14.3. Phase 13 (README and
documentation pass) is complete: tasks 13.1 (README palette documentation and
v1.1 feature documentation) and 13.2 (canonical current-state reconciliation
sweep) are complete; all three Phase 13 acceptance criteria are met; Gate 13 is
...
14 (v1.1 release) is complete: tasks 14.1-14.5 are complete; the reproducible
`artifacts/ArrTags_1.1.0.0.zip`, the v1.1 live matrix, and the release security
review pass with 0 open BLOCKER/HIGH, the `release-reviewer` gate returned
`SHIP_WITH_ACCEPTED_LIMITATIONS`, and the annotated tag `v1.1.0` is created; the
GitHub publish remains the user's
...
| 14 | v1.1 release | Complete (Phase 14 tasks 14.1-14.5 complete; `artifacts/ArrTags_1.1.0.0.zip` reproducible at 594,931 bytes / SHA-256 `85730fe7…`; the v1.1 live matrix and the release security review pass with 0 open BLOCKER/HIGH; the `release-reviewer` gate returned `SHIP_WITH_ACCEPTED_LIMITATIONS`; the annotated tag `v1.1.0` is created. The GitHub release publication remains the user's manual step) | The plugin builds and packages reproducibly at `1.1.0.0`; the full suite and the live pinned-host matrix pass; a fresh security review covers logging, SEC-5, and the settings save path; the changelog/manifest are updated and the annotated `v1.1.0` tag is created after the release-reviewer gate. |
