...
implementation task is **9.3** (task 9.1, the configuration round-trip spike, and
task 9.2, the dashboard settings page, are complete). V1.1-1 (this plan, ADR-016..ADR-020, and the GOALS.md/PLANS.md
...
| 9 | Dashboard settings UI and runtime configuration activation (v1.1) | In progress (Phase 9 tasks 9.1-9.5; tasks 9.1, 9.2, 9.3, and 9.4 complete: the get-only `Collection<T>` round-trip was proven to fail under the pinned `JsonDefaults.Options`, so `EnabledLibraries`/`Renderer.Selectors` are now settable and round-trip, `Plugin` now implements `IHasWebPages` with an embedded secret-free settings page, `Plugin.UpdateConfiguration` now validates and activates a saved candidate at runtime without a restart with last-valid snapshot and private-secret retention, and a successful replacement requests a bounded, non-blocking post-save reconciliation so existing posters re-render promptly; task 9.5 remains; resolves limitation F2) | A dashboard settings page loads and saves through the elevation-gated path, a valid change applies without restart with last-valid retention, and a successful save triggers a bounded reconciliation; the get-only `Collection<T>` round-trip is proven. Phase tag `v1.1.0-phase9`. |
...
- **Promoted to v1.1 (ADR-016):** runtime configuration replacement wiring. `ConfigurationSnapshotService.TryReplace`
  ...
  change is observed without a restart. The bounded work queue, provider/render
  concurrency limiters, freshness window, and retention interval all already
  resolve their values from the current snapshot per operation. Task 9.4 adds
  the bounded post-save re-render trigger, so a successful save also re-renders
  existing posters promptly; the final Goal A verification (task 9.5) remains.
