...
| 9 | Dashboard settings UI and runtime configuration activation (v1.1) | In progress (Phase 9 tasks 9.1-9.5; tasks 9.1 and 9.2 complete: the get-only `Collection<T>` round-trip was proven to fail under the pinned `JsonDefaults.Options`, so `EnabledLibraries`/`Renderer.Selectors` are now settable and round-trip, and `Plugin` now implements `IHasWebPages` with an embedded secret-free settings page; tasks 9.3-9.5 remain; resolves limitation F2) | A dashboard settings page loads and saves through the elevation-gated path, a valid change applies without restart with last-valid retention, and a successful save triggers a bounded reconciliation; the get-only `Collection<T>` round-trip is proven. Phase tag `v1.1.0-phase9`. |
